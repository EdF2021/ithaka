// ============================================
// Ithaka UI — Main Application Orchestrator
...
function _isForegroundChatBusy() {
  ...
  return !!window.__ithakaChatBusy
    || Date.now() < (window.__ithakaChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__ithakaQueueStreamingSubmit = now;
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__ithakaDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Ithaka Chat';
  ...
  if (_opener) window._ithakaRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('ithaka-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__ithakaSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'ithaka-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Ithaka...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__ithakaSetChatMode === 'function') {
          window.__ithakaSetChatMode('chat');
        } else {
        ...
        if (typeof window.__ithakaSetChatMode === 'function') {
          window.__ithakaSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'ithaka-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'ithaka-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('ithaka:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__ithakaQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startIthakaApp() {
  if (window.__ithakaAppStarted) return;
  window.__ithakaAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__ithakaChatBusyUntil = Math.max(window.__ithakaChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('ithaka-doc-open-' + _curSession) === '1') {
  ...
  window._ithakaBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__ithakaQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__ithakaChatBusy
        || Date.now() < (window.__ithakaChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startIthakaApp, { once: true });
} else {
  startIthakaApp();
}
