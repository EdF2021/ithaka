...
const _ACTION_RECENTS_KEY = 'ithaka-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'ithaka-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Ithaka';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Ithaka' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
