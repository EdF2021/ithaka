...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'ithaka.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__ithakaActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'ithaka-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'ithaka-doc-minimized-' + sessionId;
  ...
  let _ithakaAttachMenu = null;

  function _closeIthakaAttachMenu() {
    if (_ithakaAttachMenu) {
      _ithakaAttachMenu.remove();
      _ithakaAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_ithakaAttachMenu && !_ithakaAttachMenu.contains(e.target)) _closeIthakaAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeIthakaAttachMenu();
  }
  ...
  function _ithakaAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageIthakaAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-ithaka`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageIthakaZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-ithaka-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterIthakaAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachIthakaItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageIthakaAttachment(kind, id);
      if (!data) return;
      _afterIthakaAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeIthakaAttachMenu();
    } catch (err) {
      console.error('Failed to attach Ithaka item:', err);
      if (uiModule) uiModule.showError('Failed to attach from Ithaka');
    }
  }

  function _selectedIthakaAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-ithaka-attach-row.is-selected') || []);
  }

  function _syncIthakaAttachSelection(menu) {
    const selected = _selectedIthakaAttachRows(menu);
    const bar = menu?.querySelector?.('.email-ithaka-attach-actions');
    const count = menu?.querySelector?.('.email-ithaka-attach-count');
    const attachBtn = menu?.querySelector?.('.email-ithaka-attach-selected');
  ...
  async function _attachSelectedIthakaItems(menu) {
    const rows = _selectedIthakaAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-ithaka-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageIthakaZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageIthakaAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterIthakaAttachmentsAdded(added, zip ? 'ithaka-attachments.zip' : undefined);
      _closeIthakaAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected Ithaka items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from Ithaka');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadIthakaAttachItems(menu, kind) {
    const list = menu.querySelector('.email-ithaka-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-ithaka-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-ithaka-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncIthakaAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _ithakaAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-ithaka-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-ithaka-attach-dot" aria-hidden="true"></span>
            <span class="email-ithaka-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-ithaka-attach-main">
              <span class="email-ithaka-attach-title">${_escHtml(label)}</span>
              <span class="email-ithaka-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-ithaka-attach-dot" aria-hidden="true"></span>
            <span class="email-ithaka-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-ithaka-attach-main">
              <span class="email-ithaka-attach-title">${_escHtml(label)}</span>
              <span class="email-ithaka-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachIthakaItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncIthakaAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load Ithaka attach items:', err);
      list.innerHTML = '<div class="email-ithaka-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-ithaka-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-ithaka-attach-local">
      ...
      <div class="email-ithaka-attach-tabs">
      ...
      <label class="email-ithaka-attach-search-wrap">
        ...
        <input type="search" class="email-ithaka-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-ithaka-attach-list"></div>
      <div class="email-ithaka-attach-actions">
        <span class="email-ithaka-attach-count"></span>
        <button type="button" class="email-ithaka-attach-selected" disabled>
    ...
    _ithakaAttachMenu = menu;
    _positionIthakaAttachMenu(anchor, menu);
    menu.querySelector('.email-ithaka-attach-local')?.addEventListener('click', () => {
      _closeIthakaAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadIthakaAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-ithaka-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadIthakaAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-ithaka-attach-selected')?.addEventListener('click', () => _attachSelectedIthakaItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadIthakaAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__ithakaActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'ithaka:email-ai-reply-context:v1:';
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('ithaka-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('ithaka-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'ithaka-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('ithaka-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('ithaka-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('ithaka-suggestions-' + docId);
      if (!raw) return;
