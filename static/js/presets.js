  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('ithaka-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('ithaka-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('ithaka-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('ithaka-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('ithaka-char-sessions', JSON.stringify(charSessions));
