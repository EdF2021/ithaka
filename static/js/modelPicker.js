...
const RECENT_KEY = 'ithaka-model-recent';
const FAVORITES_KEY = 'ithaka-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__ithakaChatBusy || Date.now() < (window.__ithakaChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _collapsedProviders = new Set(_loadList('ithaka-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('ithaka-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('ithaka:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('ithaka:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `ithaka-model-favorites[0]`
