...
const COLLAPSE_KEY = 'ithaka-models-collapsed';
const FAVORITES_KEY = 'ithaka-model-favorites';
const USAGE_KEY = 'ithaka-model-usage';
const SORT_KEY = 'ithaka-model-sort';
...
export async function refreshModels(force = false) {
  ...
  try {
    ...
    if (!_cachedItems || _cachedItems.length === 0) {
      ...
      const welcomeSub = document.getElementById('welcome-sub');
      if (welcomeSub) welcomeSub.textContent = 'De bestemming is belangrijk, de reis belangrijker.';
      const welcomeTip = document.getElementById('welcome-tip');
      if (welcomeTip) {
