...
let _sortMode = Storage.get('ithaka-session-sort') || 'active'; // default to last active
...
const FOLDER_STATE_KEY = 'ithaka-folder-state';
const FOLDER_ORDER_KEY = 'ithaka-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Ithaka kept
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('ithaka-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('ithaka-session-sort', mode);
  else Storage.remove('ithaka-session-sort');
  renderSessionList();
}
