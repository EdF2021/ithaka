...
- Deploying without touching the main checkout's git (e.g. from a worktree at `origin/dev`): `docker compose -p ithaka --env-file /home/eddef/projects/ithaka/.env -f docker-compose.yml build ithaka` in the worktree, then `docker compose -p ithaka --project-directory /home/eddef/projects/ithaka up -d --no-build ithaka` — the run config (bind mounts `./data`, `./logs`) must stay the main checkout's.
