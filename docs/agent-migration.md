...
Ithaka should be able to learn from another agent without blindly trusting
...
Markdown notes, or any other agent can have its own adapter, but Ithaka only
...
- **Memory candidates** are short facts or preferences that can be reviewed
  before being saved into Ithaka memory.

This keeps Ithaka's existing memory-review flow intact while giving it better
...
The helper does not write to `data/`, call an LLM, import Ithaka modules, or
...
A future Ithaka importer should treat the manifest as untrusted user-provided
...
Nonstandard folders should be adapter details, not required Ithaka concepts.
