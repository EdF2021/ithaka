...
Ithaka keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/ithaka-backup` tool
...
```bash
# Create a snapshot → backups/ithaka-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/ithaka-backup snapshot

# List existing snapshots (most recent first)
./scripts/ithaka-backup list

# Check a tarball's integrity without extracting it
./scripts/ithaka-backup verify backups/ithaka-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/ithaka-backup restore backups/ithaka-backup-20260101-120000.tar.gz --yes
```
...
./scripts/ithaka-backup snapshot --out /mnt/nas/ithaka-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/ithaka-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/ithaka && ./scripts/ithaka-backup snapshot --out "/mnt/nas/ithaka-$(date +\%F).tar.gz"
```
