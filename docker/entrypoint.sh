...
# and "ithaka" already exists at the same id). Otherwise create.
if ! getent group "$PGID" >/dev/null 2>&1; then
    groupadd -g "$PGID" ithaka
fi
if ! getent passwd "$PUID" >/dev/null 2>&1; then
    useradd -u "$PUID" -g "$PGID" -M -s /bin/sh -d /app ithaka
fi

ITHAKA_USER="$(getent passwd "$PUID" | cut -d: -f1)"
[ -z "$ITHAKA_USER" ] && ITHAKA_USER=ithaka
...
if [ "${ITHAKA_ENABLE_HOST_DOCKER:-}" = "true" ] && [ -S "$DOCKER_SOCK" ]; then
    ...
    if [ -n "$SOCK_GID" ] && [ "$SOCK_GID" != "0" ]; then
        ...
        if [ -n "$SOCK_GROUP" ]; then
            usermod -aG "$SOCK_GROUP" "$ITHAKA_USER" 2>/dev/null || true
        fi
    fi
fi
...
"$GOSU_BIN" "$ITHAKA_USER" "$PYTHON_BIN" /app/setup.py || true

# Drop root and run the actual app. `gosu` is preferred over `su` /
# `sudo` because it cleans up the process tree (no extra shell layer)
# so signals (SIGTERM from `docker stop`) reach uvicorn directly.
exec "$GOSU_BIN" "$ITHAKA_USER" "$@"
