...
<#
  Build a portable Windows distribution for Ithaka.

  Output layout:
    dist\Ithaka\Ithaka.exe
    dist\Ithaka\static\...
    dist\Ithaka\scripts\...
    dist\Ithaka\mcp_servers\...
    dist\Ithaka\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Ithaka @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Ithaka" -ForegroundColor Green
