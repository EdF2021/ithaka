...
- Outlook.com / Microsoft 365 accounts go through the bundled `davmail` service (internal-only, hostname `davmail`) — Microsoft basic auth for IMAP/SMTP is dead, DavMail is the OAuth bridge. First-time coupling is a one-time `DAVMAIL_AUTHENTICATION=O365DeviceCode` login; the refresh token then persists in the `davmail-config` volume. Full setup and troubleshooting: `docs/email-outlook.md`.
