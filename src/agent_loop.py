...
Streaming agent loop for ithaka-ui.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Ithaka chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
GENERIC LOOPBACK to allowed Ithaka internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(r"\b(cookbook|serve|serving|served|launch|start|preset|vllm|sglang|llama\.?cpp|ollama|download|downloading|pull|cached models?|running models?|model servers?|models? (?:are )?running|what models?|model picker|gpu box|kierkegaard|ithaka|ajax|qwen|gemma|llama|mistral|minimax)\b"):
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] ithaka doc minimal memory facts=%s", len(facts))
    return {
        "role": "user",
        "content": (
            "Saved user memory facts from Ithaka Brain. These are the same "
...
def _minimal_ithaka_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the Ithaka document LoRA.
    ...
    if stream_create:
        system = (
            "You are Ithaka. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are Ithaka. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, Ithaka will answer Done."
        )
...
def _minimal_ithaka_notes_messages(messages: List[Dict]) -> List[Dict]:
    """Tiny prompt path for Ithaka notes LoRAs.

    The finetune is trained to emit Ithaka note tool calls without receiving
    the full tool schema or saved-context wrapper stack.
    """
    latest = _extract_last_user_message(messages)
    system = (
        "You are Ithaka. Handle note, todo, checklist, and reminder requests.\n"
        "You have access to the user's Ithaka notes through manage_notes.\n"
        "For 'what are my notes', 'show my notes', note searches, note creation, todos, checklists, and reminders, use the Ithaka manage_notes tool call format.\n"
...
def _minimal_ithaka_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    """Minimal fallback for Ithaka finetunes outside domain-specific paths."""
    latest = _extract_last_user_message(messages)
    system = (
        "You are Ithaka. Answer directly and briefly.\n"
        "Use Ithaka tool-call format only when the user explicitly asks you to take an action.\n"
    ...
    if active_document:
        ...
        if _is_email_doc:
            ...
            doc_ctx = (
                ...
                f'2. Replace ONLY the new reply text above `---------- Previous message ----------`. You may omit the quoted history from your tool output; Ithaka preserves everything from that separator downward automatically.\n'
...
async def _select_agent_tools(
    *,
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        ...
        logger.info("[agent-intent] ithaka doc finetune tool clamp=%s", sorted(_relevant_tools))
    elif _ody_notes_finetune_mode and _relevant_tools is not None:
        _relevant_tools = {"manage_notes", "ask_user", "update_plan"}
        logger.info("[agent-intent] ithaka notes finetune tool clamp=%s", sorted(_relevant_tools))
...
async def stream_agent_loop(
    ...
    if _ody_doc_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_ithaka_doc_messages(
            messages,
            _prompt_active_document,
        ...
        mcp_schemas = []
        logger.info(
            "[agent-intent] ithaka doc minimal prompt active active_doc=%s stream_create=%s messages=%s",
            bool(_prompt_active_document),
            _ody_doc_stream_create_mode,
            len(messages),
        )
    elif _ody_notes_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_ithaka_notes_messages(messages)
        mcp_schemas = []
        logger.info(
            "[agent-intent] ithaka notes minimal prompt active messages=%s",
            len(messages),
        )
    elif _ody_qwen_finetune_model and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_ithaka_general_messages(
            messages,
            include_memory=True,
        )
        mcp_schemas = []
        logger.info(
            "[agent-intent] ithaka general minimal prompt active include_memory=%s messages=%s",
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] ithaka doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] ithaka doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _ody_qwen_finetune_model and tool_blocks:
            ...
            if _dropped_memory_lookup:
                logger.info(
                    "[agent-intent] ithaka qwen dropped manage_memory lookup; answering from compact memory"
                )
