...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Ithaka runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Ithaka' own interpreter directory to PATH. If Ithaka itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[ithaka] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'ITHAKA_USER_SHELL="${SHELL:-}"',
        'if [ -n "$ITHAKA_USER_SHELL" ] && [ -x "$ITHAKA_USER_SHELL" ]; then',
        '  ITHAKA_USER_PATH="$("$ITHAKA_USER_SHELL" -ic \'printf "__ITHAKA_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__ITHAKA_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$ITHAKA_USER_PATH" ]; then export PATH="$ITHAKA_USER_PATH:$PATH"; fi',
        'fi',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('ITHAKA_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Ithaka.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$ITHAKA_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $ITHAKA_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$ITHAKA_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Ithaka-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('ITHAKA_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$ITHAKA_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  ITHAKA_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[ithaka] vLLM CLI: $ITHAKA_VLLM_BIN"')
    runner_lines.append('  ITHAKA_VLLM_VERSION="$("$ITHAKA_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$ITHAKA_VLLM_VERSION" ]; then echo "[ithaka] vLLM version: $ITHAKA_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('ITHAKA_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $ITHAKA_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $ITHAKA_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $ITHAKA_CMD_EXIT ==="')
        runner_lines.append('exit "$ITHAKA_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _ithaka_have_prebuilt=1
    ...
    runner_lines.append('    _ithaka_have_prebuilt=""')
    runner_lines.append('    _ithaka_arch="$(uname -m)"')
    runner_lines.append('    _ithaka_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_ithaka_arch" = "x86_64" ]; then')
    runner_lines.append('      _ithaka_pat=""')
    runner_lines.append('      _ithaka_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _ithaka_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _ithaka_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _ithaka_has_nv_inline; then')
    runner_lines.append('        _ithaka_pat="ubuntu.*cuda"')
    ...
    runner_lines.append('          ln -sf "$_ithaka_extracted" ~/bin/llama-server')
    runner_lines.append('          _ithaka_libdir="$(dirname "$_ithaka_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_ithaka_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/ithaka-llama-cpp-env')
    runner_lines.append('          _ithaka_have_prebuilt=1')
    runner_lines.append('          echo "[ithaka] Prebuilt llama-server installed at $_ithaka_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_ithaka_have_prebuilt" ] && echo "[ithaka] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_ithaka_prebuilt_url" ]; then')
    runner_lines.append('      echo "[ithaka] No matching prebuilt llama-server for this host (arch=$_ithaka_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_ithaka_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _ithaka_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _ithaka_has_nvidia_hw; then')
    ...
    runner_lines.append('    _ithaka_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[ithaka] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[ithaka] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[ithaka] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[ithaka] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _ithaka_apt_bootstrap')
    runner_lines.append('    _ithaka_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _ithaka_missing_build_deps="$_ithaka_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _ithaka_missing_build_deps="$_ithaka_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _ithaka_missing_build_deps="$_ithaka_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_ithaka_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_ithaka_missing_build_deps"')
    ...
    runner_lines.append('    _ithaka_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _ithaka_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[ithaka] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _ithaka_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _ithaka_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _ithaka_has_cudart; then')
    runner_lines.append('        echo "[ithaka] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[ithaka] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[ithaka]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[ithaka]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _ithaka_has_vulkan_device && _ithaka_has_vulkan; then')
    runner_lines.append('      echo "[ithaka] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[ithaka] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[ithaka]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[ithaka]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _ithaka_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[ithaka] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[ithaka] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[ithaka] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\ithaka-sessions"
