...
def _apply_ithaka_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Ithaka-Origin"] = ITHAKA_MAIL_ORIGIN
    if kind:
        msg["X-Ithaka-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Ithaka-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def _normalize_addr_field(field: str) -> str:
    ...
    trailing comma (e.g. `friend@example.com,`) and most MTAs reject the
    resulting `To: friend@example.com,` line as a syntax error. Collapse
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.ithaka.local>",
            "subject": subject,
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Ithaka marker header, but include the subject
                # fallback too. The fallback uses a distinct Ithaka prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Ithaka-Kind "reminder" SUBJECT "Reminder (Ithaka):"{from_clause})',
                )
    ...
    @router.delete("/ithaka/reminders")
    async def delete_ithaka_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Ithaka reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Ithaka-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Ithaka):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Ithaka):")})'))
                            # Legacy reminders created before the Ithaka
            ...
            logger.error(f"delete_ithaka_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    def _load_ithaka_attachment_source(db, kind: str, item_id: str, owner: str):
    ...
    @router.post("/compose-from-ithaka")
    async def compose_from_ithaka(data: dict, owner: str = Depends(require_owner)):
        """Stage an Ithaka document or gallery image as a compose upload."""
        ...
        try:
            ...
            db = _SL()
            try:
                src = _load_ithaka_attachment_source(db, kind, item_id, owner)
                if "path" in src:
            ...
            logger.error(f"Failed to stage Ithaka attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-ithaka-zip")
    async def compose_from_ithaka_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several Ithaka documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            try:
                ...
                return _stage_compose_bytes("ithaka-attachments.zip", content)
            finally:
                db.close()
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Failed to stage Ithaka zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", ithaka_kind=None, ithaka_ref=None,
    ):
        ...
        _apply_ithaka_headers(outer, ithaka_kind or "scheduled", ithaka_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, ithaka_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("ithaka_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="ithaka.local")
        ...
        if req.ithaka_kind:
            _apply_ithaka_headers(outer, req.ithaka_kind)
