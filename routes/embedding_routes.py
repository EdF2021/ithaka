...
def setup_embedding_routes():
    ...
    def set_endpoint(url: str = Form(...), model: str = Form(""), api_key: str = Form("")):
        ...
        # SSRF hardening: the user-supplied URL is validated by the fetch
        # helper before any outbound request — the vetted DNS resolution is
        # pinned onto the connection (no rebinding TOCTOU) and redirects are
        # re-validated per hop. Local-first means loopback/LAN endpoints are
        # allowed by default; non-HTTP(S) schemes and the cloud metadata range
        # are always rejected. Set EMBEDDING_BLOCK_PRIVATE_IPS=true for full
        # lockdown.
