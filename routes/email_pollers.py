                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Ithaka (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Ithaka" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "ithaka_kind" if "ithaka_kind" in cols else "'scheduled' AS ithaka_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                ithaka_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Ithaka-Origin"] = "ithaka-ui"
                outer["X-Ithaka-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", ithaka_kind or "scheduled")[:64]
                outer["X-Ithaka-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `ithaka-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ITHAKA_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `ithaka-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ITHAKA_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ITHAKA_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ITHAKA_INPROCESS_POLLERS=0); "
            "drive `ithaka-mail poll-scheduled` externally."
        )
        return
    import asyncio
