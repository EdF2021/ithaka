...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. Ithaka "
...
# Override at deploy time via ITHAKA_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "ithaka_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN ithaka_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("ITHAKA_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for Ithaka-generated mail (e.g. reminder, scheduled).
    ithaka_kind: Optional[str] = None
