...
def _load_setup_module():
    spec = importlib.util.spec_from_file_location("ithaka_setup_under_test", Path("setup.py"))
...
def test_create_default_admin_normalizes_env_username(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ITHAKA_ADMIN_USER", " AdminUser ")
    monkeypatch.setenv("ITHAKA_ADMIN_PASSWORD", "temporary-password")
...
def test_main_loads_admin_password_from_env_file(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ITHAKA_ADMIN_USER", raising=False)
    monkeypatch.delenv("ITHAKA_ADMIN_PASSWORD", raising=False)
    (tmp_path / ".env").write_text(
        "ITHAKA_ADMIN_USER=presetuser\nITHAKA_ADMIN_PASSWORD=fromenvfile12345\n",
        encoding="utf-8-sig",
    )
    ...
    monkeypatch.setenv("ITHAKA_SKIP_ADMIN_PROMPT", "1")
    ...
    try:
        ...
        # don't inherit them.
        os.environ.pop("ITHAKA_ADMIN_USER", None)
        os.environ.pop("ITHAKA_ADMIN_PASSWORD", None)
