"""Regression: `ithaka-calendar list` must select events that OVERLAP the
...
`ithaka-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "ithaka-calendar"
    loader = importlib.machinery.SourceFileLoader("ithaka_calendar_cli", str(path))
